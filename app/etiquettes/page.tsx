...
export default function InventairePage() {
  ...
  // --- OUTIL DE NETTOYAGE MÉCANIQUES INTELLIGENT ---
  const nettoyerMecaniques = () => {
    const invalides = jeux.filter(j => !j.mecanique || !MECANIQUES_OFFICIELLES.includes(j.mecanique.trim()));
    const uniqueInvalides = Array.from(new Map(invalides.map(item => [item.ean, item])).values());
    
    const autoUpdates: Record<string, string> = {};
    
    uniqueInvalides.forEach(jeu => {
      let suggestion = "";
      const rawMeca = jeu.mecanique ? jeu.mecanique.trim() : "";
      const normMeca = normalizeStr(rawMeca);
      const normNom = normalizeStr(jeu.nom);

      if (rawMeca) {
        // Comparaison avec les valeurs existantes dans la base de données
        let bestMatch = "";
        let bestDist = Infinity;
        
        for (const off of MECANIQUES_OFFICIELLES) {
          const normOff = normalizeStr(off);
          
          // S'il y a inclusion ou égalité parfaite sur le texte normalisé
          if (normMeca === normOff || normMeca.includes(normOff) || normOff.includes(normMeca)) {
            bestMatch = off;
            bestDist = 0;
            break;
          }
          
          // Tolérance pour les fautes de frappe
          const dist = levenshtein(normMeca, normOff);
      ...
      // Si aucune suggestion n'est trouvée (ou champ vide), on cherche "puzzle" dans le nom
      if (!suggestion || !rawMeca) {
        if (normNom.includes("puzzle")) {
          suggestion = "Puzzle";
        } else if (normNom.includes("casse tete") || normNom.includes("cassetete")) {
          suggestion = "Casse-tête";
        }
      }

      if (suggestion) {
        autoUpdates[jeu.ean] = suggestion;
      }
    });

    setMecaUpdates(autoUpdates);
                  ...
                  <div key={jeu.ean} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs font-bold text-slate-500 mt-1 flex items-center gap-2">
                        Actuelle : {jeu.mecanique ? <span className="text-red-500 bg-red-50 px-2 py-0.5 rounded border border-red-100">{jeu.mecanique}</span> : <span className="text-slate-400 italic">Vide</span>}
                      </p>
                    </div>
                    
                    <div className="shrink-0 w-full sm:w-64">
                      <select 
                        value={mecaUpdates[jeu.ean] !== undefined ? mecaUpdates[jeu.ean] : ""}
                        onChange={(e) => setMecaUpdates({ ...mecaUpdates, [jeu.ean]: e.target.value })}
                        className={`w-full font-bold text-sm rounded-xl px-3 py-2 outline-none cursor-pointer shadow-sm border-2 transition-colors ${mecaUpdates[jeu.ean] ? 'bg-emerald-50 border-emerald-300 text-emerald-800' : 'bg-slate-50 border-slate-200 text-slate-700 focus:border-black'}`}
                      >
            ...
            {jeuxMecaInvalides.length > 0 && (
              <div className="p-6 border-t-2 border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
                ...
                <button 
                  ...
                  disabled={isFixingMeca || Object.values(mecaUpdates).filter(v => v !== "").length === 0}
